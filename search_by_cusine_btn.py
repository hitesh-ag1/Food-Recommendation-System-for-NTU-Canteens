...
def search_cuisine():
        ...
        def search_data():
                ...
                dishes = search(foodnames,minrates,priceranges, veg_check, halal_check, search_type = "Cuisine")
        ...
        for i in range(0,len(di)):
              if di[i]['Cuisine'] in distinct_cus:
                    continue
              else:
                    distinct_cus.append(di[i]['Cuisine'])


        dft_val = StringVar(root)
        dft_val.set(sorted(distinct_cus)[0]) # default value

        canvas = Canvas(root, bg = 'white', height=100, width=70)

        entry1 = OptionMenu(canvas, dft_val, *sorted(distinct_cus))
